...
public class SevenActivity extends Activity {
    ...
    private void initView() {       //initView()方法头部
        ...
        write_layout.setBackgroundResource(R.drawable.bg7);     //设置书写界面背景
        ...
        try{
            InputStream is = getResources().getAssets().open("on7_1.png");      //打开第一张图片
    ...
    private synchronized void lodimagep(int j) {   //lodimagep()方法头部
        i=j;
        if (i<25){
            String name = "on7_"+i;          //设置背景图序号递增显示
    ...
    protected void dialog(){
        typedialog = false;
        AlertDialog.Builder builder=new AlertDialog.Builder(SevenActivity.this);
    ...
    public void OnYS(View v){
        if (mdialog == null){
            mdialog=new mCustomProgressDialog(this,"演示中点击边缘取消演示动画",R.drawable.frame7);
        }
        mdialog.show();
    }
